...
postgres:
	...
	$$DOCKER network create blogmon-network 2>/dev/null || true; \
	echo "Starting PostgreSQL..."; \
	$$DOCKER run -d --name blogmon-postgres \
	...
	sleep 3; \
	$$DOCKER exec blogmon-postgres pg_isready -U blogmon; \
...
postgres-reset: postgres-stop
	...
	$$DOCKER volume rm blogmon-postgres-data || true; \
	$(MAKE) postgres
...
down:
	...
	$$DOCKER rm blogmon-postgres || true; \
	$$DOCKER network rm blogmon-network 2>/dev/null || true; \
